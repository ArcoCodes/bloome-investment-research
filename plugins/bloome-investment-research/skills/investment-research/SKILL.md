...
`sell` and `primary` may be searched in either order. Keep `sell` and `primary` as separate corpus searches. Primary results have no expert/official source label, so within `primary` run separate expert-targeted and official-targeted `research_search` calls using different concepts and phrases—never `source_types`. Search experts with company or product plus customer, supplier, competitor, channel, and former-employee roles, varying demand, orders, inventory, capacity, pricing, delivery, product progress, share, and time terms. Finding official materials does not complete primary research. Expert evidence is the highest-priority requirement for completed research: the final evidence mix and report body must be expert-heavy, with multiple independent expert passages distributed across the core industry claims rather than concentrated in one section. If a core industry claim lacks expert evidence, repeat the expert search with different roles, chain positions, and wording. After exhaustive expert search, report any remaining gap instead of writing an official-only report.
...
Use `assets/template.html` as the visual source of truth; fill its placeholders and insert report content within its existing structure. Do not replace it with a newly invented card layout or a different page structure. Preserve its header, judgment block, section layout, source bar, hover-tooltip system, and existing visual language. Surface source coverage exactly once: fill the header's data-coverage placeholders (sell-side reports read, primary/industry materials read, data cutoff) and never repeat that coverage line as body prose near the top of the report. Extra coverage detail, such as a valuation cutoff date or retrieval narrative, belongs only in the closing sources-and-coverage section. Pass `report_month` to `research_synthesize` when a data cutoff is specified, for example `2026年7月`.
