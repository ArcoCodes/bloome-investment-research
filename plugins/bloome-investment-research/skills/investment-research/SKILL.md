...
8. Assemble `final_report.md` from the chapter drafts in outline order. Preserve every substantive chapter paragraph verbatim; add only the report title, front matter, transitions, and source-coverage framing needed by the existing final-report structure. Do not ask the parent or another model to rewrite the chapters into a fresh summary. Remove an exact duplicate paragraph only when the same text already survives elsewhere. Preserve decisive accepted evidence, citations, boundaries, disagreements, unresolved points, and the exact final ranking. Before rendering, verify every chapter paragraph appears in `final_report.md`.
...
`final_report.md` is an ordered assembly of complete chapter drafts, not a one-shot answer, a fresh summary of those drafts, or a dump of module memos.
...
- Preserve every substantive chapter paragraph during assembly. Improve clarity inside the chapter draft before assembly; afterward, add transitions or remove only exact duplicate paragraphs. Before delivery, compare `final_report.md` against every chapter and `decision.md`. Preserve decisive accepted evidence, citations, disagreements, boundaries, unresolved points, and the ranking recorded in `decision.md`; resolve draft contradictions before assembly.
...
Render primary evidence quotations visibly with `<blockquote class="primary-quote">` and replace `{{primary_quote_source}}` with the exact matched `evidence.json` source label. The visible source line should use the returned source party/title and publication date; keep page/line locators inside `evidence.json` for traceability, not in the reader-facing line. Never write a generic label or invent a source. Keep sell-side references as the template's `.src` hover tooltips.
