...
Begin with one reader-facing H1 report title. The first substantive section, normally the core judgment, follows as the next H1.
...
- Write primary quotations as ordinary Markdown blockquotes and end the same blockquote with `来源：<exact source party/title> · <published_at>`. The attribution remains a compact footer inside that quotation component; never emit a separate source-only block below it. Reader-facing attribution must name the actual institution, expert platform, company, regulator, or publisher and must never expose internal IDs, chunk keys, filenames, or retrieval handles. The React renderer emits visible `.primary-quote` components. Keep page/line locators in `evidence.json` for traceability, not in the visible source line. Do not use generic or invented source labels. Write inline evidence references as `{{cite:<evidence-id>}}`, using the exact `id` or `chunk_id` from `evidence.json`. The renderer replaces the marker with the standard reader-facing title and page/line locator and creates the `.src` tooltip. Exact legacy bracket citations remain supported, but do not abbreviate them to institution-and-date labels such as `[J.P. Morgan, 2026-07-02]`. Expert evidence is the highest-priority reader-facing layer: show multiple independent expert passages, and for every core industry claim with accepted expert support or challenge evidence, show at least one matched passage. One isolated quotation, one extracted sentence, or a source-bar entry is invalid; show the complete paragraph or paragraphs.
- Match quotations and source labels to the report language. In a Chinese-language report, a non-Chinese evidence passage must have a complete, faithful Chinese translation in `quote_zh`, and a non-Chinese title or expert descriptor must have a reader-facing translation in `title_zh`. Fill citation labels, tooltip headers and bodies, visible `primary-quote` blocks, and visual source lists with the localized display fields; use the original fields directly only when they already match the report language. Preserve all sentences, figures, dates, qualifiers, and paragraph boundaries. Never summarize, abridge, drop clauses, pair the translation with a long parenthetical original, or replace interior text with an ellipsis (`……`/`...`). Keep tooltip text plain, without underlines or `<u>` markup.
...
The final report should include a reader-facing source coverage section. Describe source categories naturally; do not expose internal fields such as `corpus`, `chunk_id`, or `report_id`. Summarize:
...
The outline declares each visual once as `Planned visual: <descriptive-key> — ...`; the component type exists only here.
...
Each `retrieval_rounds` entry records `corpus` and `accepted_evidence_ids`, an array of exact accepted `evidence.json` IDs or chunk IDs produced by that round. Primary retrieval must contain separate entries with `source_layer: "expert"` and `source_layer: "official"`; this field records the query intent because primary search results do not carry that source label. One primary call cannot represent both. A search round with no accepted evidence may be recorded for coverage transparency but cannot satisfy the sell-side or expert evidence completion gate. Completed institutional research requires accepted passages from at least three independently authored sell-side documents to be cited in the body. Multiple chunks from one underlying document count as one source; there is no mechanical cap on how often a genuinely decisive document may be cited, but repeated citations never count as independent corroboration. The report should assign different sources to the analytical questions they actually answer. It also requires accepted expert evidence from multiple independent origins shown as complete visible quotations.
...
Treat `report.md` as a generated mirror of `final_report.md`. Do not maintain it separately; `render_research_report` overwrites it atomically before rendering.
