...
# dependencies = []
# ///
# Adapted from TauricResearch/TradingAgents (Apache-2.0) dataflows/stocktwits.py
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="StockTwits messages for a ticker (keyless)")
    p.add_argument("symbol"); p.add_argument("--limit", type=int, default=30)
    a = p.parse_args()
    print(_json.dumps({"symbol": a.symbol.upper(), "source": "stocktwits",
                       "data": fetch_stocktwits_messages(a.symbol, limit=a.limit)}, ensure_ascii=False))
