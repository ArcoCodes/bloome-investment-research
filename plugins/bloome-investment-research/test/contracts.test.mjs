...
const contracts = new Map([
  ["skills/investment-research/assets/template.html", "f5a094aa391132986d8d6756aecf1fee65f17eaf72609ac1ef23a6987626d031"],
...
test("deep research waits behind a concise user-facing start gate", async () => {
  ...
  assert.match(skill, /End with a short, natural choice/);
  assert.match(skill, /请回复“开始”/);
  assert.match(skill, /调整研究重点、投资期限或交付方式/);
  assert.match(skill, /我会在你确认后开始研究/);
  assert.doesNotMatch(skill, /可以吗？/);
