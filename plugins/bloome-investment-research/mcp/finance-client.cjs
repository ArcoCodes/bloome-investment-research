...
async function completeResearchRun(workspace, options = {}) {
  ...
  if (typeof prepared.uploadUrl !== "string" || !prepared.requiredHeaders || typeof prepared.requiredHeaders !== "object") {
    throw new Error("Bloome Finance did not return a report publication target");
  }
  ...
  if (!upload.ok) throw new Error(`Deployable report publication failed (${upload.status})`);
